...
class CandleStickChart {
    ...
    #drawSMA() {
        ...
        const smaData = calculateSMA(this.#filteredData.slice(0, this.#filteredData.length - 1), period);
        const line = d3
            .line()
    ...
    #calculateCandleWidth() {
        ...
        this.#candleLockerWidth =
            this.#xScaleFunc((minMax[0]?.getTime() ?? 0) + this.#candleLockerWidthDate) - this.#xScaleFunc(minMax[0]?.getTime() ?? 0);
    ...
    #createYaxis() {
        ...
        //   this.remove();
        // });
        d3.selectAll(`#${this.#objectIDs.yAxisId}  g text`).attr("transform", "translate(5,0)");
        let gridColor = this.#colors.grid;
        d3.selectAll(`#${this.#objectIDs.yAxisId}  .tick line`).each(function (d, i) {
            d3.select(this).style("stroke", gridColor);
        });
        d3.selectAll(`#${this.#objectIDs.yAxisId} .tick text`).style("fill", this.#colors.tickColor);
    }
    #createXaxis() {
        let xAxis = d3
        ...
        d3.select(`#${this.#objectIDs.svgId}`)
            ...
            .call(xAxis);
        d3.selectAll(`#${this.#objectIDs.xAxisId} g text`).attr("transform", "translate(0,10)");
        let gridColor = this.#colors.grid;
        ...
        //   this.remove();
        // });
        d3.selectAll(`#${this.#objectIDs.xAxisId} .tick text`).style("fill", this.#colors.tickColor);
    }
    #createInfoText() {
        d3.select(`#${this.#objectIDs.svgId}`)
            .append("rect")
    ...
    #createToolsBtns() {
        d3.select(`#${this.id}`)
            ...
            .style("gap", "10px")
            .style("padding-right", window.innerWidth > this.#config.mobileBreakPoint ? "20px" : "0")
        ...
        d3.select(`#${this.#objectIDs.toolsBtnsContainer}`)
            ...
            .style("top", "10px")
            .style("right", "10px");
        const toolsBtnSma = document.querySelector(`#${this.#objectIDs.toolsBtnsContainer} #tools-btn-sma`);
        if (toolsBtnSma) {
  ...
  `;
        ...
        const toolsBtn0 = document.querySelector(`#${this.#objectIDs.toolsBtnsContainer} #tools-btn-0`);
        if (toolsBtn0) {
            toolsBtn0.innerHTML = `
      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 20 20">
        <g fill="none" fill-rule="evenodd" stroke="${this.#colors.deActiveTools}" stroke-linecap="round" stroke-linejoin="round" transform="matrix(0 1 1 0 2.5 2.5)">
        <path d="m3.98652376 1.07807068c-2.38377179 1.38514556-3.98652376 3.96636605-3.98652376 6.92192932 0 4.418278 3.581722 8 8 8s8-3.581722 8-8-3.581722-8-8-8"/>
        <path d="m4 1v4h-4" transform="matrix(1 0 0 -1 0 6)"/>
        </g>
      </svg>`;
        }
        const toolsBtn1 = document.querySelector(`#${this.#objectIDs.toolsBtnsContainer} #tools-btn-1`);
        if (toolsBtn1) {
            toolsBtn1.innerHTML = `
      <svg xmlns="http://www.w3.org/2000/svg" fill="${this.#colors.deActiveTools}" width="18" height="18" viewBox="2 2 30 30" id="icon">
        <defs>
          <style>
            .cls-1 {
              fill: none;
        ...
        const toolsBtn2 = document.querySelector(`#${this.#objectIDs.toolsBtnsContainer} #tools-btn-2`);
        if (toolsBtn2) {
            toolsBtn2.innerHTML = `
      <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="21" height="21" viewBox="0 0 512 512" version="1.1">
        ...
        <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
            <g id="drop" fill="${this.#colors.deActiveTools}" transform="translate(42.666667, 42.666667)">
    ...
    #modeHandler(mode) {
        this.#mode = mode;
        if (mode === "pan") {
            const element = document.querySelector(`#${this.#objectIDs.toolsBtnsContainer} #tools-btn-2 svg g g`);
            if (element) {
                element.setAttribute("fill", this.#colors.activeTools);
            }
            d3.select(`#${this.#objectIDs.toolsBtnsContainer} #tools-btn-2`).style("border", `1px solid ${this.#colors.activeTools}`);
            const toolsBtn1Svg = document.querySelector(`#${this.#objectIDs.toolsBtnsContainer} #tools-btn-1 svg`);
            if (toolsBtn1Svg) {
                toolsBtn1Svg.setAttribute("fill", this.#colors.deActiveTools);
            }
            d3.select(`#${this.#objectIDs.toolsBtnsContainer} #tools-btn-1`).style("border", `1px solid ${this.#colors.deActiveTools}`);
        }
        else if (mode === "zoom") {
            const element = document.querySelector(`#${this.#objectIDs.toolsBtnsContainer} #tools-btn-2 svg g g`);
            if (element) {
            ...
            d3.select(`#${this.#objectIDs.toolsBtnsContainer} #tools-btn-2`).style("border", `1px solid ${this.#colors.deActiveTools}`);
            const toolsBtn1Svg = document.querySelector(`#${this.#objectIDs.toolsBtnsContainer} #tools-btn-1 svg`);
            if (toolsBtn1Svg) {
                toolsBtn1Svg.setAttribute("fill", this.#colors.activeTools);
            }
            d3.select(`#${this.#objectIDs.toolsBtnsContainer} #tools-btn-1`).style("border", `1px solid ${this.#colors.activeTools}`);
        }
    }
    #handleResetZoom() {
        this.#zoomRange1 =
    ...
    #xLineHandler(d, position) {
        ...
        if (position)
            ...
            xPosition = this.#xScaleFunc(parseDate(d.date));
        ...
        if (xLabel) {
          ...
          ${xPosition >=
                ...
                : xPosition <= this.#config.xLabelWidth / 2
                    ? 0
                    : xPosition - this.#config.xLabelWidth / 2},${this.#config.svgHeight})`);
            const xLabelTextElement = document.querySelector(`#${this.#objectIDs.xLabelSelectorId} text`);
            if (xLabelTextElement) {
                xLabelTextElement.innerHTML = d3.timeFormat(this.#config.timeFormat)(this.#xScaleFunc.invert(xPosition));
            }
        }
        else {
            d3.select(`#${this.#objectIDs.svgId}`)
    ...
    #yLabelHandler(d, position) {
        ...
        if (yLabel) {
            d3.select(yLabel).attr("transform", `translate(${this.#config.svgWidth},
          ${position >=
                ...
                : position <= this.#config.yLabelHeight / 2
                    ? 0
                    : position - this.#config.yLabelHeight / 2})`);
            const yLabelTextElement = document.querySelector(`#${this.#objectIDs.yLabelSelectorId} text`);
            ...
            d3.select(`#${this.#objectIDs.svgId}`)
                ...
                .attr("transform", `translate(${this.#config.svgWidth},
              ${position >=
                (this.#config.svgHeight ?? window.innerHeight - 50) -
                    this.#config.yLabelHeight / 2
                ? (this.#config.svgHeight ?? window.innerHeight - 50) -
    ...
    #candleInfoHandler(d) {
        ...
        const candleInfoElement = document.getElementById(this.#objectIDs.candleInfoId);
        if (candleInfoElement) {
            candleInfoElement.innerHTML = `
        O <tspan style='fill:${isUp ? this.#colors.candleInfoTextUp : this.#colors.candleInfoTextDown}'>${d.open.toFixed(this.#config.decimal)}</tspan> 
        H <tspan style='fill:${isUp ? this.#colors.candleInfoTextUp : this.#colors.candleInfoTextDown}'>${d.high.toFixed(this.#config.decimal)}</tspan> 
        L <tspan style='fill:${isUp ? this.#colors.candleInfoTextUp : this.#colors.candleInfoTextDown}'>${d.low.toFixed(this.#config.decimal)}</tspan> 
        C <tspan style='fill:${isUp ? this.#colors.candleInfoTextUp : this.#colors.candleInfoTextDown}'>${d.close.toFixed(this.#config.decimal)}</tspan>
        V <tspan style='fill:${isUp ? this.#colors.candleInfoTextUp : this.#colors.candleInfoTextDown}'>${d.volume.toFixed(this.#config.decimal)}</tspan>`;
        }
        const candleInfoBackground = document.getElementById(this.#objectIDs.candleInfoIdBackground);
        ...
        if (d.long || d.short) {
            let text = "";
            if (d.long) {
                text = `Long <tspan style='fill:${this.#colors.long}'> ${d.long.toFixed(this.#config.decimal)}</tspan>`;
            }
            else {
                text = `Short <tspan style='fill:${this.#colors.short}'> ${d.short.toFixed(this.#config.decimal)}</tspan>`;
            }
            text += ` SL <tspan style='fill:${this.#colors.sl}'> ${d.sl.toFixed(this.#config.decimal)}</tspan>`;
            text += ` TP <tspan style='fill:${this.#colors.tp}'> ${d.tp.toFixed(this.#config.decimal)}</tspan>`;
            const candleInfoElement = document.getElementById(this.#objectIDs.candleInfoIdPosition);
            if (candleInfoElement) {
                candleInfoElement.innerHTML = text;
            }
            const candleInfoBackgroundPosition = document.getElementById(this.#objectIDs.candleInfoIdBackgroundPosition);
    ...
    #candleInfoLeaveHandler() {
        ...
        const candleInfoPositionElement = document.getElementById(this.#objectIDs.candleInfoIdPosition);
    ...
    #handleZoomBox() {
        ...
        if (zoomBox2)
            zoomBox2.remove();
        const containerElement = document.getElementById(`${this.#objectIDs.candleContainerId}`);
        ...
        d3.select(`#${this.#objectIDs.candleContainerId}`)
            ...
            .attr("width", this.#zoomPoint2 > this.#zoomPoint1
            ? this.#zoomPoint1
            : this.#zoomPoint2)
            .attr("x", 0)
            .attr("y", 0)
        ...
        d3.select(`#${this.#objectIDs.candleContainerId}`)
            ...
            .attr("x", this.#zoomPoint2 > this.#zoomPoint1
            ? this.#zoomPoint2
            : this.#zoomPoint1)
            .attr("y", 0)
            .attr("height", height)
    ...
    #handleZoom() {
        ...
        let leftDate = parseDate(this.#xScaleFunc.invert(minMaxZoom[0]).toISOString()).getTime();
        let rightDate = parseDate(this.#xScaleFunc.invert(minMaxZoom[1]).toISOString()).getTime();
        if (leftDate - rightDate === 0) {
            return;
        }
        let filteredData = this.data.filter((x) => {
            return (parseDate(x.date).getTime() > leftDate - this.#candleWidthDate &&
        ...
        let newZoomRange1 = parseDate(this.#xScaleFunc.invert(minMaxZoom[0]).toISOString()).getTime();
        let newZoomRange2 = parseDate(this.#xScaleFunc.invert(minMaxZoom[1]).toISOString()).getTime();
        this.#zoomFactor =
    ...
    #handlePan(location) {
        ...
        let filteredData = this.data.filter((x) => {
            return (parseDate(x.date).getTime() > newZoomRange1 - this.#candleWidthDate &&
    ...
    #handleScrollZoom(e) {
        ...
        const svgElement = document.getElementById(`${this.#objectIDs.candleContainerId}`);
        ...
        let filteredData = this.data.filter((x) => {
            return (parseDate(x.date).getTime() > left - this.#candleWidthDate &&
    ...
    #handleMouseMove(e, d) {
        ...
        if (location.x > this.#config.width)
            location.x = this.#config.width;
        if (location.y > this.#config.height)
            location.y = this.#config.svgHeight;
        //x line
        if (!this.#lockSelectorX)
            this.#xLineHandler(d, location.x);
        //y line
    ...
    #addEvenetListeners() {
        ...
        d3.select(`#${this.#objectIDs.candleContainerId}`).on("mousemove", function (e, d) {
            thisProxy.#handleMouseMove(e, d);
        });
        d3.select(`#${this.#objectIDs.candleContainerId}`).on("mouseleave", function (e, d) {
            thisProxy.#handleMouseLeave();
        });
        d3.select(`#${this.#objectIDs.candleContainerId}`).on("mousedown", function (e, d) {
            thisProxy.#handleMouseDown(e);
        });
        d3.select(`#${this.#objectIDs.candleContainerId}`).on("mouseup", function (e, d) {
            thisProxy.#handleMouseUp();
        });
        d3.select(`#${this.#objectIDs.candleContainerId}`).on("touchstart", function (e, d) {
            thisProxy.#handleMouseDown(e);
        });
        d3.select(`#${this.#objectIDs.candleContainerId}`).on("touchend", function (e, d) {
            thisProxy.#handleMouseUp();
        });
        d3.select(`#${this.#objectIDs.candleContainerId}`).on("touchmove", function (e, d) {
            thisProxy.#handleMouseMove(e, d);
        });
        d3.select(`#${this.#objectIDs.toolsBtnsContainer} #tools-btn-1`).on("click", function (e, d) {
            thisProxy.#modeHandler("zoom");
        });
        d3.select(`#${this.#objectIDs.toolsBtnsContainer} #tools-btn-2`).on("click", function (e, d) {
            thisProxy.#modeHandler("pan");
        });
        d3.select(`#${this.#objectIDs.toolsBtnsContainer} #tools-btn-0`).on("click", function (e, d) {
        ...
        d3.select(`#${this.#objectIDs.toolsBtnsContainer} #tools-btn-sma`).on("click", () => {
    ...
    #removeEventListeners() {
        if (!this.#objectIDs.candleContainerId)
            return;
        d3.selectAll(`#${this.#objectIDs.candleContainerId} .candle`)
            .on("mouseover", function (e, d) {
            null;
        ...
        d3.select(`#${this.#objectIDs.toolsBtnsContainer} #tools-btn-1`).on("click", null);
        d3.select(`#${this.#objectIDs.toolsBtnsContainer} #tools-btn-2`).on("click", null);
        d3.select(`#${this.#objectIDs.toolsBtnsContainer} #tools-btn-0`).on("click", null);
    ...
    #toggleChartType() {
        ...
        d3.select(`#${this.#objectIDs.toolsBtnsContainer} #tools-btn-toggle-chart`).text(this.isLineChart ? "Candlestick Chart" : "Line Chart");
    ...
    #drawVolumeChart() {
        ...
        const volumeGroup = d3
            ...
            .attr("transform", `translate(0, ${this.#config.svgHeight - volumeHeight})`);
        volumeGroup
            .selectAll("rect")
            .data(this.#filteredData)
            .enter()
    ...
    startLiveFeed() {
        ...
        let currentCandle = {
            date: new Date(lastDate.getTime() + this.#getTimeIntervalInMs(selectedTimeInterval)).toISOString(),
...
let chart = new CandleStickChart(window.innerWidth, window.innerHeight - 50, oneMinuteData, "chart1");
chart.draw();
// chart.startLiveFeed();
